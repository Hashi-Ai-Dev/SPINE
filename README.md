...
[![v0.2.1](https://img.shields.io/badge/version-v0.2.1-brightgreen.svg)]()
...
- **Drift** — agents accumulate scope without you noticing until the codebase is a mess
  → caught by `spine drift scan` before it ships
- **Black boxes** — you forget what you decided and why by the next session
  → solved by `spine evidence add` + `spine decision add`
- **No ground truth** — agents work from memory, not from agreed-upon direction
  → mission briefs generated by `spine brief --target claude`
...
SPINE is installed once and pointed at any repo with `--cwd`. You do NOT install SPINE into the repo you want to govern.
...
SPINE is installed once. Point it at any repo with `--cwd`.
...
| `spine brief --target claude\|codex\|openclaw` | Generate a mission brief for a specific agent |
...
.openclaw/spine.yaml    ← OpenClaw integration config (generated by spine init)
```
...
SPINE v0.2.1 is the current stable release — a post-beta stabilization release. All beta exit gates are cleared. The command surface is stable.

Phase 3B candidates (remote MCP, webhooks, web UI) are deferred. See [docs/SPINE_ROADMAP.md](docs/SPINE_ROADMAP.md) for milestone structure.
