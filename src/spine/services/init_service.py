...
class InitService:
    ...
    def run(self) -> InitResult:
        ...
        self._create_or_skip(
            result, repo_root / C.OPENCLAW_CONFIG_PATH, _OPENCLAW_CONFIG_CONTENT, base=repo_root
        )
    ...
    def _detect_conflicts(self, repo_root: Path, spine: Path) -> list[str]:
        ...
        candidates = [
            ...
            repo_root / C.OPENCLAW_CONFIG_PATH,
        ]
        return [
...
_OPENCLAW_CONFIG_CONTENT = """\
# SPINE integration settings for OpenClaw
# Generated by `spine init` — edit as needed.

spine:
  # Path to governance state directory (relative to repo root)
  state_dir: .spine

  # Load this brief at session start to ground the agent in the mission
  brief: .spine/briefs/openclaw/latest.md

  # Guidance file for all agents (runtime-agnostic)
  guidance: AGENTS.md
...
startup:
  ...
  # Generate a fresh brief for this session
  - uv run spine brief --target openclaw
"""
