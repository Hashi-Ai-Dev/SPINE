...
# ---------------------------------------------------------------------------
# Issue #45 regression: AGENTS.md template must not ship invalid commands
# ---------------------------------------------------------------------------


def test_agents_md_template_has_valid_brief_command(tmp_path: Path) -> None:
    """AGENTS.md generated by 'spine init' must use the valid brief command.

    Regression test for #45: the template previously contained
    'uv run spine brief generate ...' which is an invalid subcommand.
    The correct form is 'uv run spine brief --target claude'.
    """
    make_git_repo(tmp_path)
    run_init(tmp_path)

    content = (tmp_path / "AGENTS.md").read_text()
    # Correct command must be present
    assert "uv run spine brief --target claude" in content, (
        "AGENTS.md must contain the valid 'brief --target claude' command"
    )
    # Invalid command must NOT be present
    assert "brief generate" not in content, (
        "AGENTS.md must not contain the invalid 'brief generate' command"
    )


def test_agents_md_template_has_valid_opportunity_command(tmp_path: Path) -> None:
    """AGENTS.md generated by 'spine init' must use the valid opportunity command.

    Regression test for #45: the template previously contained
    'uv run spine opportunity add ...' which is an invalid subcommand.
    The correct form is 'uv run spine opportunity score <title>'.
    """
    make_git_repo(tmp_path)
    run_init(tmp_path)
    ...
    content = (tmp_path / "AGENTS.md").read_text()
    # Correct command must be present
    assert "uv run spine opportunity score" in content, (
        "AGENTS.md must contain the valid 'opportunity score' command"
    )
    # Invalid command must NOT be present
    assert "opportunity add" not in content, (
        "AGENTS.md must not contain the invalid 'opportunity add' command"
    )
